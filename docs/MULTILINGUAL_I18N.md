...
cache > English source** — and a `/api/translate` serverless function
...
| `js-modules/i18n-translation-client.js` | `TranslationClient`: calls `/api/translate` on demand for dynamic content (e.g. a review body), with localStorage caching and request de-duplication. |
...
- `/api/translate.js` follows `api/firebase-config.js`'s philosophy exactly:
...
2. **Filling gaps with AI**: for a key missing in a target language, a page
   ...
   hits `/api/translate`, caches the result client-side, and the app records
